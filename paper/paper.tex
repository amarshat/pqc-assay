...
\title{A Reproducible Refinement Proof of the ML-DSA Reference Forward NTT,\\
       from C to FIPS 204}
...
argued or assumed. Alongside, we report a tractability boundary met on a second implementation
(RustCrypto, via SAW over MIR): wide-domain Barrett reduction over $x<2^{46}$ does not discharge under
eager bit-blasting across six solver and tactic configurations, while the same relation in
unbounded-integer arithmetic discharges in $0.04$\,s. The proof restructurings that cross it include the
total-invariant induction used for the C overflow bound. We disclose two off-by-one defects in the
reference's documented contracts. The development reproduces from \code{make verify} (SAW plus the full
Isabelle chain, including the NTT bridge) and runs green in CI; it uses no vendor verification artifacts.
\end{abstract}
...
\begin{enumerate}
  ...
  the forward-NTT model.
  \item \textbf{A tractability boundary, and the restructuring that crosses it} (Section~\ref{sec:boundary}).
  On a second implementation (RustCrypto, via SAW over MIR), narrow-domain Barrett reduction ($x<2^{28}$)
  proves in about four seconds while the cryptographically relevant wide domain ($x<2^{46}$) does not
  complete under \code{z3}, \code{abc}, \code{cvc5}, \code{yices}, a \code{goal\_eval} rewrite pass, or a
  carry-fold reformulation. We isolate the cause with a controlled experiment: the same goal over
  $x<2^{46}$ in unbounded-integer arithmetic discharges in $0.04$\,s where the bit-vector form times out,
  locating the difficulty in bit-blasting $(x\cdot M)\gg k$ across the eager back-ends tested, not in the
  modulus or formula size. The total-invariant induction this motivates is what makes the C
  overflow-freedom bound tractable, replacing a $\sim$3000-obligation SAW unrolling with an eight-step
  proof.
