...
text \<open>Upper-leg twiddle factor: at the odd output block the exponent picks up \<open>2^(s+1)\<close>,
  which times the stride \<open>2^(6-s)\<close> is \<open>128\<close> per unit, so the root contributes a sign
  \<open>(-1)^m\<close> on top of the plain power. Uses only \<open>17^128 = -1\<close> (\<open>zwrap_cong\<close>). ML-KEM analogue
  of ML-DSA \<open>zE2_cong\<close> (wrap at 128 rather than 256).\<close>
lemma zE2_congK:
  assumes s: "s < 7"
  shows "[(17::int) ^ ((2*(2^s + brv s a)+1) * m * 2^(6-s))
        = (- 1)^m * 17 ^ ((2*brv s a+1) * m * 2^(6-s))] (mod 3329)"
proof -
  have s6: "s + (6 - s) = 6" using s by linarith
  have pkey: "(2::nat)^s * 2^(6-s) = 64" by (simp add: power_add[symmetric] s6)
  have EL: "(2*(2^s + brv s a)+1) * 2^(6-s) = 128 + (2*brv s a+1) * 2^(6-s)"
    using pkey by (simp add: algebra_simps)
  have eexp: "(2*(2^s + brv s a)+1) * m * 2^(6-s) = 128*m + (2*brv s a+1) * m * 2^(6-s)"
    using EL by (simp add: algebra_simps)
  have e1: "(17::int) ^ ((2*(2^s + brv s a)+1) * m * 2^(6-s))
      = (17 ^ 128)^m * 17 ^ ((2*brv s a+1) * m * 2^(6-s))"
  proof -
    have "(17::int) ^ ((2*(2^s + brv s a)+1) * m * 2^(6-s))
        = 17 ^ (128*m + (2*brv s a+1) * m * 2^(6-s))" by (simp only: eexp)
    also have "\<dots> = 17 ^ (128*m) * 17 ^ ((2*brv s a+1) * m * 2^(6-s))" by (simp only: power_add)
    also have "(17::int) ^ (128*m) = (17^128)^m" by (simp only: power_mult)
    finally show ?thesis .
  qed
  have c: "[((17::int) ^ 128)^m * 17 ^ ((2*brv s a+1) * m * 2^(6-s))
        = (- 1)^m * 17 ^ ((2*brv s a+1) * m * 2^(6-s))] (mod 3329)"
    by (intro cong_mult cong_pow[OF zwrap_cong] cong_refl)
  show ?thesis unfolding e1 by (rule c)
qed

text \<open>Upper-leg recursion (congruence mod q): one step at the odd output block \<open>2a+1\<close> is the
  subtractive butterfly leg. ML-KEM analogue of ML-DSA \<open>inv_form_upper\<close>.\<close>
lemma inv_formK_upper:
  fixes g :: "nat \<Rightarrow> int"
  assumes s: "s < 7" and offlo: "2 ^ (7 - s) \<le> off" and offhi: "off < 2 ^ (8 - s)"
  shows "[inv_formK (Suc s) g (a * 2 ^ (8 - s) + off)
        = inv_formK s g (a * 2 ^ (8 - s) + (off - 2 ^ (7 - s)))
          - 17 ^ ((2 * brv s a + 1) * 2 ^ (6 - s)) * inv_formK s g (a * 2 ^ (8 - s) + off)]
         (mod 3329)"
proof -
  define e where "e = 2 * brv s a + 1"
  have suc:   "8 - s = Suc (7 - s)" using s by linarith
  ...
  have L1: "inv_formK (Suc s) g (a * 2 ^ (8-s) + off)
        = (\<Sum>m'<2 ^ Suc s. g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s))
            * 17 ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s)))"
  proof -
    have "inv_formK (Suc s) g (a * 2 ^ (8-s) + off)
        = inv_formK (Suc s) g ((2*a+1) * 2 ^ (8-Suc s) + (off - 2 ^ (7-s)))" by (simp add: arw2)
    also have "\<dots> = (\<Sum>m'<2 ^ Suc s. g ((off - 2 ^ (7-s)) + m' * 2 ^ (8-Suc s))
            * 17 ^ ((2*brv (Suc s) (2*a+1)+1) * m' * 2 ^ (7-Suc s)))"
      by (rule inv_formK_ac[OF cL])
    also have "\<dots> = (\<Sum>m'<2 ^ Suc s. g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s))
            * 17 ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s)))"
      by (simp add: ssuc ssuc2 brv_double1 add.commute)
    finally show ?thesis .
  qed
  have L2: "[(\<Sum>m'<2 ^ Suc s. g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s))
              * 17 ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s)))
          = (\<Sum>m'<2 ^ Suc s. (- 1)^m'
              * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s))))] (mod 3329)"
  proof (rule cong_sum)
    fix m' :: nat assume "m' \<in> {..<2 ^ Suc s}"
    have z: "[(17::int) ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s))
          = (- 1)^m' * 17 ^ (e * m' * 2 ^ (6-s))] (mod 3329)"
      using zE2_congK[OF s, of a m'] by (simp add: e_def)
    have "[g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s))
          = g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * ((- 1)^m' * 17 ^ (e * m' * 2 ^ (6-s)))] (mod 3329)"
      by (rule cong_mult[OF cong_refl z])
    thus "[g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ ((2*(2^s + brv s a)+1) * m' * 2 ^ (6-s))
          = (- 1)^m' * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s)))] (mod 3329)"
      by (simp add: mult.left_commute)
  qed
  have L3: "(\<Sum>m'<2 ^ Suc s. (- 1)^m'
              * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s))))
          = inv_formK s g (a * 2 ^ (8-s) + (off - 2 ^ (7-s)))
            - 17 ^ (e * 2 ^ (6-s)) * inv_formK s g (a * 2 ^ (8-s) + off)"
  proof -
    have split: "(\<Sum>m'<2 ^ Suc s. (- 1)^m'
              * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s))))
          = (\<Sum>m<2 ^ s. (- 1)^(2*m)
                * (g ((off - 2 ^ (7-s)) + (2*m) * 2 ^ (7-s)) * 17 ^ (e * (2*m) * 2 ^ (6-s))))
          + (\<Sum>m<2 ^ s. (- 1)^(2*m+1)
                * (g ((off - 2 ^ (7-s)) + (2*m+1) * 2 ^ (7-s)) * 17 ^ (e * (2*m+1) * 2 ^ (6-s))))"
      using sum_pair_split[where A = "2 ^ s"
              and f = "\<lambda>m'. (- 1)^m' * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s)))"]
      by simp
    ...
    proof -
      have iac: "inv_formK s g (a * 2 ^ (8-s) + (off - 2 ^ (7-s)))
          = (\<Sum>m<2 ^ s. g ((off - 2 ^ (7-s)) + m * 2 ^ (8-s)) * 17 ^ (e * m * 2 ^ (7-s)))"
        using inv_formK_ac[OF cB, of g a] by (simp add: e_def)
      ...
      proof (rule sum.cong[OF refl])
        ...
        have b1: "(off - 2 ^ (7-s)) + (2*m) * 2 ^ (7-s) = (off - 2 ^ (7-s)) + m * 2 ^ (8-s)"
          by (simp add: B2L)
        have b2: "e * (2*m) * 2 ^ (6-s) = e * m * 2 ^ (7-s)" by (simp add: E2E)
        show "(- 1)^(2*m) * (g ((off - 2 ^ (7-s)) + (2*m) * 2 ^ (7-s)) * 17 ^ (e * (2*m) * 2 ^ (6-s)))
            = g ((off - 2 ^ (7-s)) + m * 2 ^ (8-s)) * 17 ^ (e * m * 2 ^ (7-s))"
    ...
    proof -
      ...
      proof (rule sum.cong[OF refl])
        ...
        show "(- 1)^(2*m+1) * (g ((off - 2 ^ (7-s)) + (2*m+1) * 2 ^ (7-s)) * 17 ^ (e * (2*m+1) * 2 ^ (6-s)))
            = - (17 ^ (e * 2 ^ (6-s)) * (g (off + m * 2 ^ (8-s)) * 17 ^ (e * m * 2 ^ (7-s))))"
          unfolding a1 a2 sgn by (simp add: power_add mult.assoc mult.left_commute mult.commute)
      qed
      also have "\<dots> = - (17 ^ (e * 2 ^ (6-s))
                * (\<Sum>m<2 ^ s. g (off + m * 2 ^ (8-s)) * 17 ^ (e * m * 2 ^ (7-s))))"
        by (simp add: sum_distrib_left sum_negf)
      also have "\<dots> = - (17 ^ (e * 2 ^ (6-s)) * inv_formK s g (a * 2 ^ (8-s) + off))"
        by (simp add: iac)
      finally show ?thesis .
    qed
    show ?thesis using split EVEN ODD by simp
  qed
  have c1: "[inv_formK (Suc s) g (a * 2 ^ (8-s) + off)
        = (\<Sum>m'<2 ^ Suc s. (- 1)^m'
              * (g ((off - 2 ^ (7-s)) + m' * 2 ^ (7-s)) * 17 ^ (e * m' * 2 ^ (6-s))))] (mod 3329)"
