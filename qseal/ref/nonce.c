...
int qseal_nonce_seen(const qseal_nonce_store_t *s, const uint8_t r[QSEAL_REQID_LEN]) {
    ...
    for (uint8_t i = 0; i < QSEAL_STORE_CAP; i++) {
        int eq = 1;
        for (uint32_t j = 0; j < QSEAL_REQID_LEN; j++)
            if (s->slots[i][j] != r[j]) eq = 0;
...
int qseal_nonce_accept(qseal_nonce_store_t *s, int ok, const uint8_t r[QSEAL_REQID_LEN]) {
    int is_seen = qseal_nonce_seen(s, r);
    int is_full = (s->count >= QSEAL_STORE_CAP);
    int a = (ok != 0) && !is_seen && !is_full;
    if (a) {
        /* append r at index count (constant-index guarded scan, matching the model's rebuild) */
        for (uint8_t i = 0; i < QSEAL_STORE_CAP; i++)
            if (i == s->count)
                for (uint32_t j = 0; j < QSEAL_REQID_LEN; j++)
                    s->slots[i][j] = r[j];
...
int qseal_nonce_accept_noconsume(qseal_nonce_store_t *s, int ok, const uint8_t r[QSEAL_REQID_LEN]) {
    ...
    int is_full = (s->count >= QSEAL_STORE_CAP);
    int a = (ok != 0) && !is_seen && !is_full;
    /* BUG: never records r, so the same validated request is accepted again (replay). */
    return a ? 1 : 0;
}
